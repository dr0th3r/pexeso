        ...
        const unsubscribe = auth.onAuthStateChanged(user => {
            ...
            if (user === null) {
                ...
                getDoc(docRef).then((userDoc) => {
                    if (userDoc.exists()) {
                        const userPacks = Object.entries(userDoc.data().packs).map(([key, value]) => {
                            return {
                                id: key,
                                title: value.title,
                        ...
                        $userData.chosenPack = $userData.packs.find(pack => pack.id === userDoc.data().chosenPackId) || $userData.packs[0];
                    } else {
                        ...
                        let filteredPacks = $userData.packs.filter(pack => defaultPacks.some(defaultPack => defaultPack.id !== pack.id));
                        ...
                        setDoc(doc(usersRef, user.uid), firebaseUser).then(() => {
